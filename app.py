...
def get_one_movie(id):
    ...
    if len(all_reviews) == 0:
        return jsonify("This Movie has no Reviews Yet!")
    avg = total / len(all_reviews)
    return jsonify({"movie": movie_schema.dump(movie), "avg_rating": avg})
...
# Delete A Review from Movie
@app.route("/review/delete/<id>", methods=["DELETE"])
def delete_review(id):
    review_to_delete = db.session.query(Review).filter(Review.id == id).first()
    db.session.delete(review_to_delete)
    db.session.commit()
    return jsonify(review_schema.dump(review_to_delete), "Review was Properly Deleted!")
